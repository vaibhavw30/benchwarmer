...
- [ ] **M3 — paper session (LIVE).** Run a full pre-game slate on `PaperVenue`; confirm via the Telemetry JSONL that books update, `skip`/`take`/`quote` fire sanely, and paper positions/P&L evolve reasonably. **Zero real orders** are placed in this mode. Needs Kalshi creds + an in-season slate; still deferred. **Offline stand-in exists:** `tools/paper_session.cpp` (target `paper_session`) replays a recorded fixture against the exact `StrategyEngine → RiskManager → PaperVenue → Telemetry` pipeline with a given `fair_values.json`, no network/creds — used to verify the recalibrated signal changes paper decisions (maker quotes shift with the boosted fair value). It does not exercise the live WS gateway or settlement P&L (the fixture only accumulates, so realized P&L stays 0 with no round-trip).
